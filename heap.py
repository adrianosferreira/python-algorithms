class Heap:
    ...
    def pop(self):
        ...
        while True:
            if self.get_left_child(curr) < len(self.data) and self.data[curr] > self.data[self.get_left_child(curr)]:
                temp = self.data[curr]
                self.data[curr] = self.data[self.get_left_child(curr)]
                self.data[self.get_left_child(curr)] = temp
                curr = self.get_left_child(curr)
            elif self.get_right_child(curr) < len(self.data) and self.data[curr] > self.data[self.get_right_child(curr)]:
                temp = self.data[curr]
